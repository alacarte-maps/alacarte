...
shared_ptr<Configuration> ConfigMockup::Config(char* argv[], int argc)
{
	...
	#define OPT(_name, _shortcut)	(string(_name).append(",").append(_shortcut)).c_str()
		...
		config_desc.add_options()
			...
			(OPT(opt::server::access_log, "a"),	value<string>()/*->value_name("path")*/,													"file where server access will be logged")
			(OPT(opt::server::path_to_default_style, "d"),	value<string>()/*->value_name("path")*/,										"default stylesheet")
			(OPT(opt::server::path_to_default_tile, "t"),	value<string>()/*->value_name("image")*/,										"default tile")
