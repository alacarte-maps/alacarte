...
shared_ptr<TestConfig> TestConfig::Create()
{
	...
	->add<string>(opt::server::path_to_default_tile,(getTestDirectory() / "/default.png").string())
	...
	->add<string>(opt::server::style_source, 		(getTestDirectory() / "/input/style-src/").string());
}
