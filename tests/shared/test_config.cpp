...
shared_ptr<TestConfig> TestConfig::Create()
{
	...
	->add<std::string>(opt::server::path_to_default_tile,(getTestDirectory() / "../data/default.png").std::string())
	//->add<std::string>(opt::server::path_to_geodata, 	(getTestDirectory() / "/input/karlsruhe_big.carte").std::string())
	...
	->add<std::string>(opt::server::style_source, 		(getTestDirectory() / "../data/mapcss/").std::string());
}
